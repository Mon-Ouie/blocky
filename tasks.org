** TODO FIX M-x not focusing input on the prompt
** TODO FIX trash drawing children

** TODO [#A] Improve the basic command-shell and prompt so that in-engine development is easier. 
*** [#A] start using terminal output for status message output
****  [#A] hook #'message minibuffer messages/notifications (message workspace, or different buffers?) 
*** [#A] generic LISP command evaluation shell

*** [#A] terminal window can be resized
