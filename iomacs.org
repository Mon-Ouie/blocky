* iomacs.org --- hypermedia metaprogramming with org-babel
...
IOMACS is a format-agnostic media creation and processing tool
suitable for making games and other interactive applications. 

IOMACS is based on [[http://www.gnu.org/software/emacs][GNU Emacs]]. It delegates content processing to
external [[http://lispgames.ath.cx/index.php/UsefulApplications][applications]] like the GIMP, Pure Data, ImageMagick, FFMPEG,
SBCL, and so on. IOMACS uses [[http://en.wikipedia.org/wiki/Digital_asset_management][Digital Asset Management]] (DAM)
...
describing the files. Because IOMACS knows what applications are
...
"Iomacs". The name is short for "[[http://en.wikipedia.org/wiki/Hypermedia][hypermedia]] programming".
...
the raw version of this workbook, [[http://dto.github.com/notebook/iomacs.org][iomacs.org]]

The github page for this workbook is http://github.com/dto/iomacs.
...
My experiments with Iomacs are an attempt to apply [[http://orgmode.org/][Orgmode]]
...
Iomacs is based on [[http://orgmode.org][Orgmode]] and [[http://orgmode.org/worg/org-contrib/babel/][Org Babel]], which are both [[http://www.gnu.org/software/emacs][GNU Emacs]]
packages. Being familiar with these programs will help understanding
the rest of this document.

* Hypermedia programming with Iomacs
...
#+begin_src emacs-lisp
(defface iomacs-chunk-header-face '((t (:foreground "red" :bold t :weight bold))) "Face for chunk header lines.")
(defvar iomacs-chunk-header-face 'iomacs-chunk-header-face)
  
  (defvar iomacs-chunk-regexp "^#\\+\\(source:\\|srcname:\\|function:\\)")
  
  (defun* iomacs-fontify-chunk (limit)
    (while (re-search-forward iomacs-chunk-regexp limit :noerror)
      (let ((beg (match-beginning 1))
            (end (match-end 1)))
        (add-text-properties beg end (list 'display (propertize (match-string 1) 'face iomacs-chunk-header-face)
                                           'font-lock-fontified t)))))
#+end_src

#+source: visible-ellipsis
#+begin_src emacs-lisp
  (defface iomacs-hidden-face '((t (:foreground "yellow" :underline "red"))) "Face for hidden iomacs text.")
  (defvar iomacs-hidden-face 'iomacs-hidden-face)
#+end_src


#+source: fontified-chunks
#+begin_src emacs-lisp
  (defun iomacs-fontify-blocks ()
    (interactive)
    (save-excursion 
      (goto-char (point-min))
...
#+begin_src emacs-lisp
 (add-hook 'org-font-lock-hook #'iomacs-fontify-chunk)
#+end_src

#+source: turn-off-visible-chunks
#+begin_src emacs-lisp
 (remove-hook 'org-font-lock-hook #'iomacs-fontify-chunk)
#+end_src

#+source: turn-on-visible-ellipsis
#+begin_src emacs-lisp
 (setf org-ellipsis iomacs-hidden-face)
#+end_src

* Processes for creating and transforming chunks

* How the Iomacs source is made
:PROPERTIES:
:ID: 13a1e5df-8185-4633-9d62-ae9cab244f07
:END:

Using Babel, the Emacs Lisp source code for Iomacs itself is extracted
from this workbook (or "tangled", in literate programming
terminology), to produce an output file called =iomacs.el=. 
...
To make =iomacs.el= from this workbook, execute the following elisp code
...
Assuming =iomacs.el= is somewhere in your Emacs load-path, you can load
...
in the output file "iomacs.el", a chunk must be referenced somewhere in
...
#+begin_src emacs-lisp
  (htmlfontify-buffer nil "iomacs-pretty.html")
#+end_src 
