** TODO visually indicate clicked send blocks
** TODO fix on-select listener not working to get keystrokes to the prompt
** TODO union fields to hold union of %methods and %excluded-fields and so on
** TODO listener history should show only commands?
** TODO define-visual-macro (via defblock?)
** TODO defblock with-target : schema has socket for message target, socket for list body
...
** TODO [#A] Also show clickable previous commands in listener log
