** TODO interface flags, like colored tape flags with white txt 
** TODO do not need topleft triangle now that shaded input area always visible?
** TODO fix top and bottom patch border off-by-one pixel errors on different machines (use 0.5)?
** TODO make it easier to grab parent widgets of an entry (allow drag from entry  label)
...
** TODO [#B] hotkey for focusing on terminal prompt (click also)
** TODO fix menus being drawn under stuff
** TODO [#A] plus button decorator for lists to add objects (fields, methods, etc)
** TODO [#A] Improve tree widget
*** TODO [#A] better indication of open/closed... go gradually darker as deeper?
*** TODO [#B] draw standard svg or bitmap icons for errors, informations, questions, and TREE ARROWS
...
** TODO [#B] fix texture edge handling for indicators:
...
** TODO [#C] define faces with bold, italic, etc
** TODO [#A] defblock tree: a generic folding hierarchical list widget with indentation and headlines, a la orgmode 
** TODO defblock defsprite --- defining sprites
