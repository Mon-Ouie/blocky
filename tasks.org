* TODO KEEP DOING THE SIMPLEST THING THAT COULD POSSIBLY WORK

** TODO F1 help F
** TODO middle-click drag is a simple gesture menu with up down left=back right=forward
** TODO [#A] Working map editor; maps are wiki page/worlds
...
** TODO [#A] browser in the menu, for choosing what to define methods on (or blocks from)
** TODO [#A] world add-object [ladybug new]
** TODO [#A] local variable palettes that can also name the func/closure
** TODO [#A] wikify blocky so that each method and each block type
...
**** TODO with colons, it's special and refers to something 
**** TODO index:
**** TODO help:
**** TODO packagename:
