** TODO Extended argument lists as in Genera
** TODO Simple data entry blocks based on prompt (add capability to base?)
** TODO Generic block type should be able to send any message, with typed arg block widgets for entering the values
** TODO [#A] global listener at bottom, M-x to bring up/focus
** TODO [#A] turtle graphics quickie examples
** TODO [#B] reasonable emacs configuration for ioforms dev (imenu etc)
** TODO [#A] DEMO VIDEO some working menu items with "send" block
** TODO [#A] Fix menu behavior: properly check other menu widgets in menubar hit before menu
