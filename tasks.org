** TODO Fix the way wiki page resources are stored into separate files...
** TODO Fix backward insertion of lines from error message into listener
** TODO Improve the basic command-shell and prompt so that in-engine development is easier. 
** TODO general purpose resizer/scroller decorator thinger
**** lower-right-corner resize handle decorator

*** TODO trigger context menu items with click, make block with control-click
*** start using terminal output for status message output
****  hook #'message minibuffer messages/notifications (message workspace, or different buffers?) 
*** TODO Allow user to move entries easier via yellow triangle tab
*** TODO pop up shell on M-x

** TODO Finish implementing the core visual programming language in OOP.LISP
