...
*** TODO contains
** TODO generic way to wrap functions?
** TODO review all IOFORMS engine features for blocks to make
** TODO [#A] Make dialogs for all menu options in file:system.lisp 
** TODO [#A] Task/process abstraction built into blocks exec model, simple coroutines?
** TODO [#B] Hide optional arguments (general horizontal list block. can also be used with APPLY block.
** TODO [#A] incorporate turtle graphics code into base block prototype (for implementing move/turn/point-in-direction 
** TODO [#A] more scratch/byob blocks
** TODO [#A] test raw string entry
** TODO [#A] project settings dialog for configuring *screen-width* and *resizable* etc
** TODO [#A] rework simple collision detection--- abstract it out and let objects handle it
...
** TODO [#A] dialog boxes are fancy SEND blocks that send to *system*
** TODO [#A] start using terminal output for status message output
...
** TODO [#A] BIGFIX: in cell/move and sprite/move , compute whether a move would cause a collision
** TODO [#A] and then only allow the move to go a certain distance. return number moved
** TODO [#A] then "resting contact" will work; the object won't try to move anymore (if gravity.)
