...
nodes" are stored in a list called %INPUTS. (The percent-sign prefix
refers to a field value of the current object.)

How a given object implements the EVALUATE method will influence
whether and when that object's %INPUTS are themselves evaluated, as
with a Lisp macro. Similarly, methods like DRAW can decide how,
whether, and when to draw a block's children.
